...
public class hknpConvexPolytopeShape extends hknpConvexShape {
	...
	public boolean readFromStream(HKXReaderConnector connector, ByteBuffer stream, int classOffset)
		...
		for (int i = 0; i < size; i++) {
			planes[i] = new NifVector4(stream, classOffset + 64 + offset + (i * 16));//16 bytes per vec4
		}

		file = connector.data.setup(classOffset + 68);
		file.get(bSize);
		file.get(bOff);
		size = ByteUtils.getUInt(bSize);
		...
		for (int i = 0; i < size; i++) {
			faces[i] = new hknpConvexPolytopeShape.Face(connector, stream,
					classOffset + 68 + offset + (i * hknpConvexPolytopeShape.Face.size));//16 bytes per vec4
		}
		...
		for (int i = 0; i < size; i++) {
			indices[i] = Byte.toUnsignedInt(stream.get(classOffset + 72 + offset + (i * 1)));
		}
